...
public class CBC implements ICipherMode {
    ...
    public byte[] encryptWithMode(byte[] text, byte[] IV, List<String> parameters, ISymmCipher algorithm, int blockSize) {
        ...
        int length = text.length / blockSize;

        for (int i = 0; i < length; ++i) {
            int startIndex = i * blockSize;
            byte[] block = new byte[blockSize];
            System.arraycopy(text, startIndex, block, 0, blockSize);

            // Шифруем результат XOR текущего блока и результата шифрования предыдущего блока
            byte[] processedBlock = algorithm.decryptBlock(BitOperations.byteArrayXOR(block, previousBlock));
...
//                });
        IntStream.range(0, cipheredText.length / blockSize)
                ...
                .forEach(i -> {
                    ...
                    byte[] previousBlock = (i == 0) ? IV : new byte[blockSize];
                    if (i != 0) {
                        System.arraycopy(cipheredText, (i - 1) * blockSize, previousBlock, 0, blockSize);
                    }

                    int startIndex = i * blockSize;
                    byte[] currentBlock = new byte[blockSize];
                    System.arraycopy(cipheredText, startIndex, currentBlock, 0, blockSize);

                    // XOR с предыдущим зашифрованным блоком
                    byte[] processedBlock = BitOperations.xor(previousBlock, algorithm.decryptBlock(currentBlock));
                    System.arraycopy(processedBlock, 0, result, startIndex, processedBlock.length);
                });
        return result;
    }
